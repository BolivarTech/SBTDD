...
@pytest.fixture
def repo_with_drift(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """State=red + HEAD=refactor: triggers phase-ordering inversion drift."""
    claude_dir = tmp_path / ".claude"
    claude_dir.mkdir()
    planning_dir = tmp_path / "planning"
    planning_dir.mkdir()
    ...
    def fake_run(cmd: list[str], timeout: int = 0, cwd: str | None = None):  # type: ignore[no-untyped-def]
        ...
        return SimpleNamespace(returncode=0, stdout="abc1234|refactor: dummy\n", stderr="")

    monkeypatch.setattr("status_cmd.subprocess_utils.run_with_timeout", fake_run)
    return tmp_path


def test_status_reports_drift_and_exits_3(
    repo_with_drift: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    import status_cmd
    from drift import DriftReport

    # Force drift detection to return a synthetic report (isolation from drift internals).
    def fake_drift(*a, **k):  # type: ignore[no-untyped-def]
        return DriftReport(
            state_value="red",
            git_value="refactor",
            plan_value="[ ]",
            reason="synthetic drift for test",
        )
    ...
    try:
        mp.setattr("status_cmd.detect_drift", fake_drift)
        rc = status_cmd.main(["--project-root", str(repo_with_drift)])
    finally:
        mp.undo()
    out = capsys.readouterr().out
    assert rc == 3
...
def test_status_drift_report_includes_three_values(
    repo_with_drift: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    import status_cmd
    from drift import DriftReport
    ...
    try:
        mp.setattr("status_cmd.detect_drift", fake_drift)
        status_cmd.main(["--project-root", str(repo_with_drift)])
    finally:
        mp.undo()
    out = capsys.readouterr().out
    assert "red" in out
