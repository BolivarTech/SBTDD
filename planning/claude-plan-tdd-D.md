...
**Architecture:** Python 3.9+ stdlib only. Se anade un dataclass (`AutoRunAudit`) para formalizar el schema JSON de `auto-run.json`, reemplazando los dict-writes ad-hoc. Se anade un helper `_parse_version_string` en `dependency_check.py` y un `_REQUIRED_VERSION_REGEX` (patron por binario). Se anade `ensure_nextest_experimental_env()` en `reporters/rust_reporter.py` para detectar el flag antes de lanzar el pipeline. Se enriquecen `_write_magi_conditions_file` (pre_merge_cmd) y `_write_auto_run_audit` (auto_cmd) con resumenes estructurados de conditions accepted/rejected. `resume_cmd` gana una rama nueva en `_decide_delegation` para detectar `magi-conditions.md` pendiente y dirigir al usuario. Todos los cambios son TDD-Guard-amigables y siguen el commit-prefix mapping de sec.M.5.

**Scope addition vs. Milestone A (frozen) — `errors.MAGIGateError`:** Milestone A freezes `errors.py` as public contract, but Finding 1 (Melchior iter 1) flagged the regex-based message parsing in Task 4 as fragile. The correct fix is to augment `MAGIGateError` with typed attributes (`accepted_conditions`, `rejected_conditions`, `verdict`, `iteration`) — a **non-breaking additive change** (keyword-only defaults, base `__init__` still accepts a single `message`). This is a deliberate, minimal exception to the frozen-module policy, justified because it eliminates a fragile string parser rather than replacing one heuristic with another. The change is:

1. Documented here as a scope addition.
2. Contained entirely in Task 4 (no other milestone module modifies `errors.py`).
3. Fully backwards compatible: any pre-existing raiser passing only `message` still works unchanged; the new attributes default to empty / `None`.
4. Accompanied by a dedicated regression test that constructs `MAGIGateError` both ways (legacy positional, new kw-only).

After this change, Task 4 reads the typed attributes directly instead of parsing `str(exc)`, eliminating the coupling of audit-trail correctness to the exact wording of the exception message.
...
├── errors.py                 # MODIFIED (scope addition): MAGIGateError +typed kw-only attrs (Finding 1, Plan D iter 1)
├── reporters/
...
- **Tasks 2, 3, 11, 13 (regression-pinning tests):** `test:` in a **single commit** (no Red/Green split). The test passes on first run because it pins behavior that already exists (INV-25, INV-26 schema conformance, integration journeys). Precedent: Plans A-C use this pattern for Tasks 8, 20, 23, 26, 30, 34 of Plan A. Per CLAUDE.local.md sec.3 and sec.M.5 row 1, adding a test file = `test:` regardless of first-run outcome. Explicit note (Finding 6, Plan D iter 1): these tasks document the regression-pinning nature in their narrative body, not as a separate exception buried in commit policy.
- **Task 4 (scope addition — `errors.MAGIGateError` kw-only attrs):** the Green commit aggregates three related modifications (`errors.py` augmentation + `pre_merge_cmd.py` raise-site updates + `auto_cmd.py` handler rewrite) into a single `feat:` commit since they form one atomic change: the exception contract changes and both its raisers and its handler adapt in the same commit. Splitting them would leave the tree in a state where `pre_merge_cmd` raises with attributes that `auto_cmd` does not yet read, i.e., a broken intermediate. The header "Scope addition" block documents the errors.py exception to the Milestone-A freeze.
- **Tasks 14-16 (documentacion, Fase 4):** `docs:` para cambios que son puramente docstrings cross-reference / comentarios.
...
| `errors.py` | Scope addition (see header): `MAGIGateError` augmented with kw-only typed attributes — non-breaking, additive | `test_auto_cmd_exit8.py::test_magi_gate_error_typed_attributes` |
...
Objetivo: codificar un schema frozen para `.claude/auto-run.json`. Hoy `_write_auto_run_audit` acepta `dict[str, object]` — cualquier typo produce silent corruption. Introducimos `AutoRunAudit` como `@dataclass(frozen=True)` con campos tipados + `from_dict`/`to_dict` + `validate_schema`. La firma de `_write_auto_run_audit` se cambia a **`AutoRunAudit` solo** (no mas union con `dict`) — Plan D iter 2 Caspar WARNING: la rama de back-compat para `dict` silenciosamente bypass-eaba validacion y dejaba que callers futuros persistieran payloads sin schema check. Como el plugin esta pre-1.0 y los unicos callers viven dentro del plugin mismo, removemos la rama por completo. Task 17 agrega un grep-check sobre `_write_auto_run_audit(.*dict` para garantizar que no reaparezca en el futuro.

**Downstream consumers (dependency contract — Finding 5, Plan D iter 1):** Task 4 populates `accepted_conditions`, `rejected_conditions`, and `tasks_completed` from the gate-block path. Those four fields (counts plus the `magi_gate_blocked` status string in `_ALLOWED_AUTO_RUN_STATUSES`) are defined in THIS task so Task 4 can just construct an `AutoRunAudit` with the values it computes; it never adds fields, never mutates an existing instance. This subsection makes the cross-task contract explicit.
...
(Plan D iter 2 removed the `from typing import Union` line previously added here — the `_write_auto_run_audit` signature no longer accepts a dict, so Union is no longer needed.)
...
Modify `_write_auto_run_audit` to require `AutoRunAudit` strictly (no dict back-compat — Plan D iter 2 Caspar):
...
Because the dict back-compat branch has been REMOVED in this task (Plan D iter 2 Caspar), all three call sites MUST be migrated atomically in this same step — leaving any one unmigrated would produce a TypeError / ValidationError at runtime. Grep-verify before committing:

```bash
grep -n "_write_auto_run_audit(" skills/sbtdd/scripts/auto_cmd.py
```

Every match must pass an `AutoRunAudit(...)` expression, never a dict literal.
...
Objetivo: regresion test asegurando que `auto_cmd`, `pre_merge_cmd` y `close_phase_cmd` NUNCA invocan `git push`, `git merge`, `gh *`, ni subprocess con comando que contenga `pr` como primer argumento. Para cerrar el gap del iter 1 (Finding 3 — spy solo sobre `run_with_timeout` ignora cualquier `subprocess.run`/`subprocess.Popen` directo), y el gap del iter 2 Caspar WARNING (el spy de iter 1 seguia ignorando `subprocess.call`, `subprocess.check_call`, `subprocess.check_output`, `os.system`, y no detectaba el uso de `shell=True`), extendemos el spy a los 7 entry points conocidos + assert explicito de `shell=False`.
...
def _assert_no_remote_ops(calls: list[list[str]]) -> None:
    for argv in calls:
        ...
        assert exe not in _FORBIDDEN_EXECUTABLES, (
            f"INV-25 violated: invoked {exe} (argv={argv})"
        )
        ...
        assert "gh pr" not in joined, (
            f"INV-25 violated: invoked `gh pr` via {argv}"
        )
...
def spy_all_subprocess(monkeypatch: pytest.MonkeyPatch) -> list[list[str]]:
    ...
    def _assert_no_shell_true(kwargs: dict[str, Any]) -> None:
        assert not kwargs.get("shell", False), (
            f"INV-25 violated: subprocess invocation used shell=True "
            f"(kwargs={kwargs})"
        )
...
**Depends on:** Task 1 (the `AutoRunAudit` dataclass and `_write_auto_run_audit(AutoRunAudit)` signature must be defined first — Plan D iter 2 Caspar WARNING made this dependency explicit). The fixture-load tests reference `auto_cmd.AutoRunAudit` and the `accepted_conditions` / `rejected_conditions` / `tasks_completed` fields; executing Task 3 before Task 1 would surface only `AttributeError` noise, not the regression-pin value.
...
**Files:**
- Modify: `skills/sbtdd/scripts/errors.py` (scope addition — `MAGIGateError` kw-only typed attributes; see header "Scope addition" block)
- Modify: `skills/sbtdd/scripts/pre_merge_cmd.py` (raise sites set the new attributes)
- Modify: `skills/sbtdd/scripts/auto_cmd.py` (`main`'s `MAGIGateError` handler + Phase 2 returns a completion count)
- Create: `tests/test_auto_cmd_exit8.py`

**Depends on:** Task 1 (uses `AutoRunAudit` dataclass with the `accepted_conditions`, `rejected_conditions`, `tasks_completed` fields defined there).

Objetivo: cuando `auto_cmd` llega a exit 8 (MAGIGateError), el audit trail anadido en Task 1 debe registrar `accepted_conditions` y `rejected_conditions`. Hoy los unicos signals son `status + error`. Tomamos los counts de **atributos tipados** en `MAGIGateError` (no string parsing) y los registramos como valores primarios en `AutoRunAudit`.

Adicionalmente, el mensaje a stderr debe incluir estos counts en una linea concisa.

**Why typed attributes instead of regex (Finding 1, Plan D iter 1):** parseando `str(exc)` acopla la correctness del audit trail a la redaccion exacta del mensaje. Un cambio de tipeo en `pre_merge_cmd._loop2` (ej. "2 accepted condition(s)" -> "accepted: 2 condition(s)") romperia `_parse_condition_counts_from_msg` silenciosamente, y el audit trail seguiria reportando 0/0 sin senal al usuario. Augmentando `MAGIGateError` con atributos tipados (`accepted_conditions: tuple[str, ...]`, `rejected_conditions: tuple[str, ...]`, `verdict: str | None`, `iteration: int | None`) obtenemos binding compile-time: mypy verifica la forma, tests fallan si la firma cambia, y el audit-trail lee estructurado (len/contenido) en vez de parsear strings. Ver "Scope addition" en el header.
...
```bash
head -n 10 skills/sbtdd/scripts/errors.py | grep -n "from __future__ import annotations"
```

Expected: a non-empty match on one of the first 10 lines. If the import is missing, STOP the task and add it as the first non-comment line before proceeding.

After the edits in later steps are in place, also verify the file compiles on Python 3.9:

```bash
python -m py_compile skills/sbtdd/scripts/errors.py
```
...
def test_auto_cmd_exit8_strong_no_go_records_zero_counts(
    ...
    # tasks_completed=0 when the file is absent.
    def fake_phase2(ns: Any, state: Any, cfg: Any) -> Any:
        return state
...
class MAGIGateError(SBTDDError):
    """MAGI verdict below threshold or STRONG_NO_GO — exit 8.
...
```python
    # Gate-block branch (accepted conditions still pending):
    raise MAGIGateError(
        f"MAGI iter {iteration} produced {len(accepted)} accepted "
        f"condition(s); apply them via `sbtdd close-phase` and re-run "
        f"`sbtdd pre-merge`. See {conditions_path}.",
        accepted_conditions=tuple(accepted),
        rejected_conditions=tuple(rejected),
    ...
    # Threshold-not-met (iteration cap reached) branch:
    raise MAGIGateError(
        f"MAGI gate not passed after {iteration} iterations",
        accepted_conditions=tuple(last_accepted),
        rejected_conditions=tuple(last_rejected),
...
def _phase2_task_loop(
    ...
    """Run the task loop. Each task close incrementally rewrites
    ``.claude/auto-run.json`` so a mid-loop raise preserves the partial
    ``tasks_completed`` count (raise-safe audit — Plan D iter 2 Caspar)."""
    tasks_completed = 0
    started = _now_iso()
    auto_run = Path(ns.project_root) / ".claude" / "auto-run.json"
    while state.current_task_id is not None:
        state = _advance_one_task(ns, state, cfg)  # runs red/green/refactor
        tasks_completed += 1
        in_progress_audit = AutoRunAudit(
            schema_version=_AUTO_RUN_SCHEMA_VERSION,
            auto_started_at=started,
            auto_finished_at=None,
            status="success",  # tentative; handler overwrites on failure
            verdict=None,
            degraded=None,
            accepted_conditions=0,
            rejected_conditions=0,
        ...
        _write_auto_run_audit(auto_run, in_progress_audit)
    return state
```

The `MAGIGateError` except-clause in `main` reads `exc.accepted_conditions` / `exc.rejected_conditions` directly (length-based), reads `tasks_completed` from the last persisted audit on disk, and rewrites the audit with terminal status:

```python
    except MAGIGateError as exc:
        accepted_count = len(exc.accepted_conditions)
        rejected_count = len(exc.rejected_conditions)
        # Source tasks_completed from the last incremental audit write in
        # Phase 2 (Plan D iter 2 Caspar — raise-safe partial count). If no
        # audit file exists yet (Phase 2 never ran a task), default to 0.
...
Helper `_read_audit_tasks_completed(path: Path) -> int`:
...
```bash
git add skills/sbtdd/scripts/errors.py skills/sbtdd/scripts/pre_merge_cmd.py skills/sbtdd/scripts/auto_cmd.py
git commit -m "feat: enrich exit-8 audit in auto_cmd via typed MAGIGateError attrs"
```
...
Modify `run_pipeline` to call `ensure_nextest_experimental_env()` unconditionally at the top of its body (Finding 7, Plan D iter 1 — no `check_env` escape hatch leaks test concerns into the production API). The signature stays exactly as it was pre-Milestone D:
...
At the top of `run_pipeline` body (before any subprocess dispatch):
...
**Heuristic, not a synchronisation primitive (Findings 4 + 8, Plan D iter 1 — revised in Plan D iter 2 Caspar):** the double-read-with-sleep design is intentionally best-effort — it catches the common case (an actively-running `auto` that is mid-write) but cannot guarantee no race slips through when a write lands entirely between the two reads. The docstring documents this tradeoff explicitly (see the Green step below). Strict file locking would be more robust but costs a third-party dep or OS-specific syscalls, which the stdlib-only constraint on hot paths (INV-21) forbids.
...
```bash
grep -rn "_write_auto_run_audit(" skills/sbtdd/scripts/ | grep -Ev "AutoRunAudit\(|AutoRunAudit\.from_dict|AutoRunAudit\]"
```

Expected: zero matches. If any line survives the filter, inspect — each caller MUST pass an `AutoRunAudit` instance or an expression whose type is `AutoRunAudit`. Legacy `dict` literal calls (e.g., `_write_auto_run_audit(path, {"status": "success", ...})`) must be converted to `AutoRunAudit(...)` constructions before acceptance.

Also verify the `Union` typing import was removed from `auto_cmd.py` if it is no longer used:

```bash
grep -n "from typing import Union" skills/sbtdd/scripts/auto_cmd.py
```
...
- **8 modulos existentes endurecidos** en `skills/sbtdd/scripts/` (auto_cmd, dependency_check, reporters/rust_reporter, reporters/ctest_reporter, pre_merge_cmd, resume_cmd, init_cmd, and errors — the last is a scope-addition exception to the Milestone-A freeze documented in the header). Ningun modulo nuevo.
- **1 nueva clase public** (`AutoRunAudit` en `auto_cmd.py`) — unica superficie de API nueva del milestone. `MAGIGateError` keyword-only attributes are an additive extension (back-compat preserved). Todo el resto son funciones privadas (prefijo `_`).
- **13 test files nuevos** bajo `tests/`:
  - `test_auto_run_audit.py` (7 tests — Task 1)
  - `test_auto_cmd_inv25.py` (4 tests — Task 2; expanded per Finding 3, Plan D iter 1)
  - `test_auto_cmd_inv26.py` (4 tests — Task 3)
  - `test_auto_cmd_exit8.py` (3 tests — Task 4; typed-attrs regression + success + STRONG_NO_GO)
...
- ~30 commits atomicos con prefijos sec.M.5:
  ...
  - Tasks 1, 5, 6, 7, 8, 9, 10, 12, 17: multi-commit `test:` → `feat:`/`fix:` → opcional `refactor:`.
  - Task 4: `test:` (Red — fails because `MAGIGateError` lacks kw-only attrs) → `feat:` (Green — single aggregated commit touching `errors.py` + `pre_merge_cmd.py` + `auto_cmd.py` per Task 4's rewrite rationale).
  - Tasks 2, 3, 11, 13: `test:` (regression-pinning tests; single commit, no Red split — see commit policy exceptions above).
...
- `_write_auto_run_audit` now requires an `AutoRunAudit` instance; the dict back-compat branch was REMOVED (Plan D iter 2 Caspar WARNING). All callers in Task 1's Step 7 are migrated atomically in the same commit to keep the tree green. Pre-1.0 the removal is safe; Task 17 Step 4b grep-checks against regression.
- `run_pipeline` signature unchanged — Finding 7 (Plan D iter 1) removed the proposed `check_env` escape hatch. The pre-check runs unconditionally; tests monkeypatch the env var.
- `MAGIGateError(message)` legacy positional-only calls continue to work unchanged — new attributes are keyword-only with safe defaults (Finding 1 rewrite, Plan D iter 1).
- `_phase2_task_loop` return type is UNCHANGED (still `SessionState`). Plan D iter 1 had proposed narrowing to `tuple[SessionState, int]`, but Plan D iter 2 Caspar WARNING showed mid-loop raises would lose the partial count. Revised design uses incremental audit writes after each task close, so the partial count survives on disk regardless of whether the loop raises.
- No public API removed; no public API signature narrowed.

**8. Plan D iter 1 findings applied:**

| Finding | Severity | Disposition |
|---------|----------|-------------|
| 1 — Task 4 regex extraction fragile | Melchior WARNING | Applied — typed attrs on `MAGIGateError` (scope addition documented in header) |
| 2 — `state._tasks_completed_count` on frozen dataclass | Melchior CRITICAL | Applied — iter 2 revised to incremental audit writes (see iter 2 Caspar below) |
| 3 — Task 2 INV-25 spy too narrow | Melchior WARNING | Applied — iter 2 extended to 7 entry points + shell=True (see iter 2 Caspar below) |
| 4 — Task 10 10ms race window narrow | Melchior WARNING | Applied — iter 2 switched to deterministic monkeypatch (see iter 2 Caspar below) |
| 5 — Task 1 / Task 4 cross-task coupling | Balthasar WARNING | Applied — Task 1 defines all fields Task 4 consumes; Task 4 Files block lists the dependency |
| 6 — Tasks 2/3/11/13 regression-pinning `test:` | Balthasar WARNING | Applied — commit-policy exceptions section + per-task narrative |
| 7 — Task 7 `check_env=False` escape hatch | Balthasar WARNING | Applied — removed parameter, tests use `monkeypatch.setenv` / `delenv` |
| 8 — Task 10 concurrency heuristic | Balthasar WARNING | Applied (folded into Finding 4 — same docstring) |

**9. Plan D iter 2 findings applied (Caspar — 1 CRITICAL + 7 WARNINGs):**

| Finding | Severity | Disposition |
|---------|----------|-------------|
| Python 3.9 `tuple[str, ...]` runtime compat | Caspar CRITICAL | Applied — Task 4 Step 0 asserts `from __future__ import annotations` is present in `errors.py` + runs `python -m py_compile` on Python 3.9 |
| `tasks_completed` under-reports on mid-Phase-2 raise | Caspar WARNING | Applied — replaced `_phase2_task_loop` tuple return with incremental audit writes via `_write_auto_run_audit` after each task close; handler re-reads the last persisted count via `_read_audit_tasks_completed` |
| Task 10 concurrent-state race test is wall-clock-flaky | Caspar WARNING | Applied — test now monkeypatches `Path.stat` so the two reads see different mtimes unconditionally; no `time.sleep` dependency |
| AutoRunAudit dict back-compat bypasses validation | Caspar WARNING | Applied — removed the dict branch from `_write_auto_run_audit`; signature now strict `AutoRunAudit` only; Task 17 Step 4b grep-check guards regression |
| INV-25 spy misses `shell=True` + `os.system` + `subprocess.call/check_call/check_output` | Caspar WARNING | Applied — Task 2 spy extended to all 7 entry points with explicit `shell=False` assertion and hard-fail on `os.system` |
| Heuristic race detection gaps on loaded CI | Melchior+Balthasar WARNING | Applied via Caspar's deterministic monkeypatch (same fix as iter 2 Task 10) |
| Dict back-compat removal deadline | Balthasar WARNING | Applied via Caspar's removal (same fix as iter 2 Task 1) |
| Task 3 ordering dependency not explicit | Caspar WARNING | Applied — Task 3 now has an explicit "Depends on: Task 1" line before Objetivo |
